...
/**
 * This script has been modified by Alex Skrypnyk (alex.designworks@gmail.com)
 *  - ereg_replace was replaced with preg_replace. Otherwise, ereg_replace
 *    throws a warning in PHP 5 and it breaks images output.
 *  - handling absolute URLs
 *  - code formatted according to Drupal style.
 */
...
if (isset($_GET['img'])) {
  ...
  // Is URL absolute?
  if (strpos($source_image, '://') !== FALSE) {
    $parsed_url = parse_url($source_image);
    $source_image = $parsed_url['path'];
  }
...
// Save any alpha data that might have existed in the source image and
// disable blending.
...
//
// This is quite simple really. There are 127 available levels of alpha,
// so we just step-through the reflected image, drawing a box over the top,
// with a set alpha level.
// The end result? A cool fade.
// There are a maximum of 127 alpha fade steps we can use, so work out the
// alpha step rate.
